...
function tokenizeParagraph(lines, mode) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (i > 0) all2.push({ type: "text", value: "\n" });
    const line = lines[i];
    if (line != null) all2.push(...tokenizeLine(line, mode));
  }
  return all2;
}
...
function resolveWikiLink(target, allSlugs) {
  ...
  const exact = allSlugs.find((s2) => s2.toLowerCase() === lower);
  if (exact) return `/${exact}`;
  const targetFile = lower.split("/").pop() ?? lower;
  const byFile = allSlugs.find((s2) => {
    const parts = s2.toLowerCase().split("/");
  ...
  return {
    ...
    htmlPlugins(ctx) {
      const cooklangRehype = () => (tree, file) => {
        ...
        const html3 = buildRecipeHTML(file.data.cooklang, fm, locale, slugs);
        const fragment = fromHtml(html3, { fragment: true });
        tree.children = fragment.children;
      };
